    ...
    def create_board(self):
        for row in range(ROWS):
            ...
            for col in range(COLS):
                if col%2 == ((row+1)%2): # squares with pieces on it
                    if row<3:
        ...
        for r in range(start, stop, step):
            ...
            if current == 0: #found empty square
                if skipped and not last:
                    break #if skipped to blank square and can't move anymore
                elif skipped:
                    moves[(r,left)] = last + skipped
                else:
                    moves[(r,left)] = last # add as legal move

                if last: #check the possibility to skip again from a blank space
                    if step == -1:
                        row = max(r-3,0)
                    else:
                        row = min(r+3, ROWS)
            ...
            elif current.color == color: #if trying to move on our piece
                break
            else: #other colors' square, safe to move if empty square behind
                last = [current]
        ...
        for r in range(start, stop, step):
            ...
            if current == 0: #found empty square
                if skipped and not last:
                    ...
                    moves[(r,right)] = last # add as legal move
